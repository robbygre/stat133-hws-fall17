...
``` r
# your test
pythagoras2 <- function(a) {sqrt(a^2 + a^2)}
pythagoras2(5)
```
...
``` r
miles <- c(1:9, seq(from = 10, to = 100, by = 10))
df <- data.frame("Miles" = miles, "Kms" = miles2kms(miles))
df
```
...
df2 <- data.frame("Liters" = liters, "Gallons" = liters2gallons(liters))
df2
```
